...
Constraint implementation function for DED method with various possible constraints."""
    if ctype[0]=='s':
        ...
        if ctype=='ssn':
            ...
            return MBGAIM(omega, H, c, eta,Tk,Boltzmann,evals, evecs,6e-4,np.array([-1,0,1])),True
        else:
            return MBGAIM(omega, H, c, eta,Tk,np.exp(-abs(evals[find_nearest(np.diag(np.conj(evecs).T@n.data@evecs),np.conj(vecs)@n.data@vecs.T)]-evals[0])/Tk),evals, evecs,6e-4,np.array([-1,0,1])),True
    elif ctype[0]=='n':
