...
Constraint implementation function for DED method with various possible constraints."""
    if ctype[0]=='s':
        ...
        if ctype=='ssn':
            ...
            return MBGAIM(omega, H, c, eta,Tk,Boltzmann,evals, evecs,5.5e-4,np.array([-2,-1,0,1,2])),True
        else:
            return MBGAIM(omega, H, c, eta,Tk,np.exp(-abs(evals[find_nearest(np.diag(np.conj(evecs).T@n.data@evecs),np.conj(vecs)@n.data@vecs.T)]-evals[0])/Tk),evals, evecs,5.5e-4,np.array([-2,-1,0,1,2])),True
    elif ctype[0]=='n':
