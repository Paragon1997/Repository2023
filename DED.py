...
if __name__=='__main__':
    ...
    filenames,DOST,omegap,labelnames=tqdm(['Lorentz2p0U','Lorentz2p3U','Lorentz3p3U','Lorentz4p3U','Lorentz5p3U','Lorentz4p3U%2N'],position=0,leave=False,desc='No. sampling Dist. SAIM DED',bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}'),np.zeros((len(filenames),int(input[-2]["N"]*input[-2]["poles"]/200)-1),dtype='float'),np.zeros((len(filenames),int(input[-2]["N"]*input[-2]["poles"]/200)-1),dtype='float'),['$\\rho_0,\it{U=0,n=2}$','$\\rho_0,\it{U=3,n=2}$','$\\rho_0,\it{U=3,n=3}$','$\\rho_0,\it{U=3,n=4}$','$\\rho_0,\it{U=3,n=5}$','$\\rho_0,\it{U=3,n=4},N\\%$2']
    for i,file in enumerate(filenames):
        nd,_,fDOS,Lor,omega,selectpT,selectpcT,tsim=DEDlib.main(**input[i])
    ...
    filenames,DOST,labelnames=tqdm(['cN4p0U','cN4p1_5U','cN4p3U','cN4p4_5U','cN4p6U','cN4p7_5U'],position=0,leave=False,desc='No. SAIM DED U sims',bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}'),np.zeros((len(filenames),1001),dtype='float'),['$\it{U=0.0}$','$\it{U=1.5}$','$\it{U=3.0}$','$\it{U=4.5}$','$\it{U=6.0}$','$\it{U=7.5}$']
    for i,file in enumerate(filenames):
    ...
    filenames,DOST,labelnames=tqdm(['cN4p_15G','cN4p_3G','cN4p_45G','cN4p_6G','cN4p_75G'],position=0,leave=False,desc='No. SAIM DED Gamma sims',bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}'),np.zeros((len(filenames),1001),dtype='float'),['$\Gamma\it{=0.15}$','$\\Gamma\it{=0.30}$','$\\Gamma\it{=0.45}$','$\\Gamma\it{=0.60}$','$\\Gamma\it{=0.75}$']
    for i,file in enumerate(filenames):
        nd,_,DOST[i],Lor,omega,selectpT,selectpcT,tsim=DEDlib.main(**input[i])
        DEDlib.DOSplot(DOST[i],Lor,omega,file,labelnames[i])
        DEDlib.textfileW(omega,np.ravel(selectpT),np.ravel(selectpcT),DOST[i],file)
    ...
    eigsel=tqdm([False,True],position=0,leave=False,desc='No. selection type sims',bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}')
    for l,sel in enumerate(eigsel):
        ...
        for j,r in enumerate(radius):
            filenames,DOST,labelnames=tqdm(['GrapheneCirc'+str(r)+'r1_5U','GrapheneCirc'+str(r)+'r3U','GrapheneCirc'+str(r)+'r4_5U','GrapheneCirc'+str(r)+'r6U'],position=2,leave=False,desc='No. U variation sims',bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}'),np.zeros((len(filenames),4001),dtype='float'),['$\it{U=1.5}$','$\it{U=3.0}$','$\it{U=4.5}$','$\it{U=6.0}$']
        ...
        for k,pos in enumerate(posimp):
            ...
            for j,imp in enumerate(posb):
                filenames,DOST,labelnames=tqdm(['GrapheneNR'+structname[k]+str(imp)+'pos1_5U','GrapheneNR'+structname[k]+str(imp)+'pos3U','GrapheneNR'+structname[k]+str(imp)+'pos4_5U','GrapheneNR'+structname[k]+str(imp)+'pos6U'],position=3,leave=False,desc='No. U variation sims',bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}'),np.zeros((len(filenames),4001),dtype='float'),['$\it{U=1.5}$','$\it{U=3.0}$','$\it{U=4.5}$','$\it{U=6.0}$']
